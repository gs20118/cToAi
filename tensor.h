...
namespace cai {
    ...
    public:
        ...
        Tensor<U> convert(){ //따라가지 못함
            Tensor<U> o(dim, func::newArray_(dim, shape));
            auto pos = o.initi();
        ...
        Tensor get(const std::vector<cai::Range> &pos) const{
            ...
            Tensor ret(newDim, newStride, newShape, data, newOffset, graddata, grad_func);
            ret.indexed = true;
            return ret;
        }
        ...
        Tensor grad() const{
            ...
            Tensor<T> ret(dim, func::newArray_(dim, stride), func::newArray_(dim, shape), graddata, offset);
            ret.graded = true;
            return ret;
        }
